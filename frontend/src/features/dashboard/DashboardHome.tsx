  ...
  return (
    <div>
            ...
            return (
              <div key={m} className="group relative flex h-full flex-1 items-end justify-center gap-[3px]">
                <div
                  ...
                  className="w-full rounded-t-[3px] bg-[#2084FA]/85 transition-colors group-hover:bg-[#2084FA]"
                  style={{ height: `${ha}%` }}
                />
                <div
                  title={`${m} PO: ${fmtLkr(a)} | Non-PO: ${fmtLkr(b)}`}
      ...
      <div className="mt-2 flex items-center gap-4 text-[11px] text-slate-500">
        <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-sm bg-[#2084FA]" /> PO spend</span>
...
export function DashboardHome({ data, user, period, onPeriod, onNavigate }: DashboardHomeProps) {
  ...
  const channelColors = ['#2084FA', '#2084FA', '#f59e0b', '#8b5cf6', '#0ea5e9', '#f43f5e'];
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-8 py-3">
                <Donut
                  segments={bucketRows.map((b, i) => ({
                    value: b.value,
                    color: ['#2084FA', '#8b5cf6', '#f59e0b', '#f97316', '#ef4444'][i]!,
                  }))}
                  size={144}
                  thickness={12}
            ...
            {(
              ...
              <button
                ...
                className={`py-3 px-1 text-xs font-medium border-b-2 transition-colors ${
                  intelTab === t.key ? 'text-[#2084FA] border-[#2084FA] font-semibold' : 'text-slate-500 border-transparent hover:text-slate-800'
                }`}
              >
                {t.label}
              </button>
