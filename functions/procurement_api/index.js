...
// The v1 vendor quote portal is opened from an emailed magic link; the token
// in the URL is the credential (see v1/router.js), so it carries no session.
const isAuthExempt = (p) => AUTH_EXEMPT_PATHS.includes(p) || p.startsWith('/api/v1/portal/');
...
const BUILD_VERSION = '5.0.0-procureflow-react';
...
app.use(async (req, res, next) => {
  if (isVendorPortalPath(req.path) || isAuthExempt(req.path)) return next();
...
// /api/v1/auth/me is the React client's boot handshake: it must answer
// "setup required" and "not invited" itself rather than be refused here.
const SETUP_PATHS = ['/api/sync-user', '/api/setup', '/api/v1/auth/me'];
...
// Invite (or link) a workspace user. Shared by the legacy POST /api/users and
// the v1 surface, so there is exactly one place that talks to Catalyst user
// management and one rollback path.
async function inviteWorkspaceUser(req, { Email, FullName, RoleID, ProfileID, ApprovalLimit }) {
  let registeredCatalystUserId = null;
  try {
    if (!Email || !FullName) throw new AppError('Email and FullName are required.', { code: 'MISSING_FIELDS' });
    ...
    return { user: newUser, invitationSent, linkedExisting };
  } catch (err) {
...
app.post('/api/users', requireAdmin, async (req, res) => {
  try {
    const result = await inviteWorkspaceUser(req, req.body || {});
...
// Update a workspace user. Shared by PUT /api/users/:id and the v1 surface so
// the last-administrator guard lives in one place.
async function updateWorkspaceUser(req, userId, { FullName, RoleID, ProfileID, ApprovalLimit, Status }) {
  {
    ...
    if (Status !== undefined || ProfileID !== undefined) {
      ...
      const [targetRows, profileRows, activeRows] = await Promise.all([
        zcql.executeZCQLQuery(
          `SELECT * FROM Users WHERE ROWID = '${zqRaw(userId)}' AND OrgID = '${zqRaw(req.orgId)}'`
        ),
...
app.put('/api/users/:id', requireAdmin, async (req, res) => {
  try {
    res.json(await updateWorkspaceUser(req, req.params.id, req.body || {}));
  } catch (err) {
    fail(res, err);
  }
});
...
// ==========================================
// /api/v1 — the surface the React client uses (see v1/router.js)
// ==========================================
const { createV1Router } = require('./v1/router');
